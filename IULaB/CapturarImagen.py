...
def PantallaImagen():
    ...
    def Seleccionar_Imagen(label):
        ...
        #predict = melanoma_model.predict(Image)
        print("Push Button")
        file_path = filedialog.askopenfilename()
        image = cv2.imread(file_path)
        # Redimensionar la imagen al tamaño deseado (por ejemplo, 128x128)
        image = cv2.resize(image, (128, 128))
        # Asegurarse de que la forma sea (1, 128, 128, 3) para una imagen en color o (1, 128, 128, 1) para una imagen en escala de grises
        if len(image.shape) == 2:
            # Si es una imagen en escala de grises, agregar un canal de color
            image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)

        image = np.expand_dims(image, axis=0)
        lbl_text="LA IMAGEN FUE CLASIFICADA COMO:"
        
        #predict = melanoma_model.predict(image)
        texto = "Espere un momento, los resultados se están obteniendo."    
        messagebox.showinfo("Cargando...", texto)
        """
        if str(predict) == "[[0. 1. 0.]]":
            lbl_text += "COMMON NEVUS" 
        elif str(predict) == "[[1. 0. 0.]]":
            lbl_text += "ATYPICAL NEVUS" 
        elif str(predict) == "[[0. 0. 1.]]":
            lbl_text += "MELANOMA" 
        print(str(predict))
        label.configure(text=lbl_text)"""
